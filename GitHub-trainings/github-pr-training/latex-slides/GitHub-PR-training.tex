...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\begin{itemize}
			...
			\item This is a far superior way to document tasks compared to emails or in-person meetings!
		\end{itemize}
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\begin{itemize}
			\setlength\itemsep{.5em}
			\item Create branch on GitHub.com \hyperlink{new-branch}{\beamergotobutton{Step-by-step}}
			\item We create a branch to have a space where work on this task does not interfere with work on other tasks
			\item Name the branch after the task and add the issue number as suffix - for example: \texttt{pr-training-11}
		\end{itemize}
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\begin{itemize}
			...
			\item You need at least one commit - Create/edit a README describing the task
		\end{itemize}
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\begin{itemize}
			...
			\item Make comments on specific lines of as much as possible - line comments are much easier to keep track of
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\begin{itemize}
			\setlength\itemsep{.5em}
			\item Make sure that the PR is well documented - this is where future team members will read how something was done and why
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\begin{itemize}
			...
			\item \textbf{Develop branches} - a branch that is a high level task that will have several \textit{feature} branches
		\end{itemize}
...
\begin{frame}
	...
	\begin{minipage}[t][5cm][t]{\textwidth}
		\begin{itemize}
			...
			\item \textit{Develop} branches often does not have an issue - the \textit{feature} branches off of it will
			\item Include \textit{develop} branch name as prefix to all \textit{feature} branches of it
		\end{itemize}
	\end{minipage}  
\end{frame}
...
\begin{frame}
	...
	\begin{minipage}[t][5cm][t]{\textwidth}
		\begin{itemize}
			\setlength\itemsep{.5em}
			\item Create as many \textit{feature} branches of the \textit{develop} branch as you need
			...
			\item Create \textit{feature} branches of the \textit{develop} branch any time - even after it progresses
		\end{itemize}
	\end{minipage}  
\end{frame}
