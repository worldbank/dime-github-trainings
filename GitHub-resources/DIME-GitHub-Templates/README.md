...
* [.gitignore template](#dime-analytics-gitignore-template) - a _.gitignore_ file controls which types of files will be shared in the GitHub repository
...
All repositories should always have a _.gitignore_ file that controls which files are uploaded to the repository. Examples of files that should not be uploaded to GitHub are:
...
1. Once you have typed `.gitignore`, a button will show up that says `Choose .gitignore: None`. Click that button and select any template. It does not matter which template you select as we will replace the content with the DIME Analytics template in the next step. We are only doing this to make sure the file is set up correctly.
...
1. Replace the content in your _.gitignore_ file with the content in [DIME Analytics template](https://raw.githubusercontent.com/worldbank/dime-github-trainings/master/GitHub-resources/DIME-GitHub-Templates/gitignore_template.txt) and then commit this file to the repository.
...
It is common that repositories for research projects need to share empty folders. For example, graph file formats are binary and should therefore not be shared in the repository, instead they should be generated by code. Let's say that the code saves the graphs in a folder called `outputs\graphs`. This folder exists in your clone but not in anyone else's clone since to Git it is empty and therefore not shared as all content of this folder is ignored. This means that anyone else running your code will get an error where those graphs are created as the code expects the `outputs\graphs` folder, but no-one but you have that folder.

Somehow we need a way to share these folders, and the way to do that is to add a placeholder file.
...
There are several ways this template can be copied to your repository. However it is copied, it should always be renamed to `README.md`. That is a special name in GitHub, and it will be displayed in the browser when anyone navigates to the folder on GitHub.com where this file sits.
