...
Below is an example set-up that explains the main rules of how access is given through teams and child teams on GitHub.com. Black boxes are teams, green boxes indicate which repos those teams are added to, and grey boxes are users that are added as members to different teams.
...
* `User A` does not have access to `repo-gx1` but `User C` has access to `repo-p1`
* In this example there is no difference to be added to `Team Parent` and `Team Child` meaning that `User A` and `User B` have the same type of access. This is because "_Team Child_" does not have its own repo. 
* `User A` and `User B` do not have access to the repos `repo-gx1`, `repo-gy1` or `repo-gy2`
* All users in this example have access to the repository `repo-p1` since this repo is in the parent team.
